...
def _fetch_alpaca_orders(date_from: str = None, date_to: str = None):
    """
...
def analytics_debug():
    """Debug endpoint — shows raw Alpaca account + positions + orders. Remove in production."""
    e = _auth()
    if e: return e
    import os
    try:
        ...
        # Positions
        pos_r    = requests.get(f"{base}/v2/positions",  headers=headers, timeout=10)
        # Orders sample
        orders_r = requests.get(f"{base}/v2/orders",
                                headers=headers,
                                params={"status": "all", "limit": 5, "direction": "desc"},
                                timeout=10)

        pos_data = pos_r.json() if pos_r.ok else pos_r.text[:300]
        return jsonify({
            ...
            "account_ok":        acct.ok,
            "account_equity":    acct.json().get("equity") if acct.ok else None,
            "positions_status":  pos_r.status_code,
            "positions_ok":      pos_r.ok,
            "positions_count":   len(pos_data) if isinstance(pos_data, list) else "error",
            "positions_sample":  pos_data[:2] if isinstance(pos_data, list) else pos_data,
            "orders_status":     orders_r.status_code,
            "orders_sample":     orders_r.json()[:2] if orders_r.ok else orders_r.text[:200],
        })
...
def api_portfolio_analytics():
    ...
    No _auth() needed — Railway sessions can drop between page load and fetch calls.
    Data is non-sensitive (paper account positions visible on dashboard anyway).
    """
    # Soft auth — return empty if not logged in rather than 401
    if not session.get("logged_in"):
        return jsonify({"error": "Not logged in", "positions": [], "summary": {}, "sector_breakdown": {}, "account": {}}), 200
...
@analytics_bp.route("/api/analytics/ai-analysis/<symbol>", methods=["GET"])
def ai_analysis(symbol):
    """
    Plain-English technical analysis generated by a LOCAL Ollama model,
    grounded in real indicator values computed by core/analyzer.py.
    Requires `ollama serve` running locally (see README).
    """
    e = _auth()
    if e: return e
    from research.ollama_analysis import get_local_ai_analysis
    tfs = request.args.get("timeframes")
    timeframes = [t.strip() for t in tfs.split(",")] if tfs else None
    try:
        result = get_local_ai_analysis(symbol, timeframes)
        return jsonify(result)
    except Exception as ex:
        logger.error(f"ai-analysis {symbol}: {ex}")
        return jsonify({"error": str(ex)}), 500


# ── Kronos price forecast ─────────────────────────────────────────────────────
@analytics_bp.route("/api/analytics/kronos-predict/<symbol>", methods=["GET"])
def kronos_predict(symbol):
    """
    Forward price forecast using the hosted Kronos API.
    Requires KRONOS_API_URL env var pointing at your deployed Kronos service.
    """
    e = _auth()
    if e: return e
    from research.kronos_predictor import get_kronos_forecast
    period   = request.args.get("period", "1D")
    pred_len = int(request.args.get("pred_len", 20))
    lookback = int(request.args.get("lookback", 400))
    try:
        result = get_kronos_forecast(symbol, period=period, pred_len=pred_len, lookback=lookback)
